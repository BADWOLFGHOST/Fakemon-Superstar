class Battle
      ...
      2.times do |side|   # Each side in turn
        ...
        sideCounts.each_with_index do |_count, i|
          if !requireds[i] || requireds[i] == 0
            case side
            when 0
              raise _INTL("玩家方的训练家{1}没有可以让精灵上场的位置（开始{2}对{3}的对战时）",
                          i + 1, @sideSizes[0], @sideSizes[1])
            when 1
              raise _INTL("对手方的训练家{1}没有可以让精灵上场的位置（开始{2}对{3}的对战时）",
                          i + 1, @sideSizes[0], @sideSizes[1])
            end
          end
          next if requireds[i] <= sideCounts[i]   # Trainer has enough Pokémon to fill their positions
          if requireds[i] == 1
            raise _INTL("玩家方训练家{1}没有可以上场的精灵", i + 1) if side == 0
            raise _INTL("对手方训练家{1}没有可以上场的精灵", i + 1) if side == 1
          end
  ...
  def pbEndOfBattle
    ...
    when 2, 5
      ...
      PBDebug.log("")
      if @internalBattle
        pbDisplayPaused(_INTL("你的手上没有可以战斗的精灵！"))
        if trainerBattle?
          case @opponent.length
          when 1
