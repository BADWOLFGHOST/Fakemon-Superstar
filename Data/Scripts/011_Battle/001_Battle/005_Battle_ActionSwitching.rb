class Battle
  ...
  def pbCanSwitchIn?(idxBattler, idxParty, partyScene = nil)
    ...
    if !pbIsOwner?(idxBattler, idxParty)
      if partyScene
        ...
        partyScene.pbDisplay(_INTL("你不能用你的精灵替换{1}的精灵！", owner.name))
      end
      return false
    end
      ...
      @battlers.each do |b|
        ...
        if !pbOwnedByPlayer?(idxBattler)   # Opponent/ally is switching in
          ...
          if @internalBattle && @switchStyle && trainerBattle? && pbSideSize(0) == 1 &&
            ...
            if pbDisplayConfirm(_INTL("{1}正准备派出{2}。要替换精灵吗？",
          ...
          switch = false
          if pbDisplayConfirm(_INTL("使用下只精灵？"))
            switch = true
          else
  ...
  def pbMessagesOnBattlerEnteringBattle(battler)
    ...
    if battler.shadowPokemon?
      ...
      pbDisplay(_INTL("噢！一个黑暗精灵！")) if battler.opposes?
    end
  end
