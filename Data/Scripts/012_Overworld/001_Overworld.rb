...
def pbCheckAllFainted
  if $player.able_pokemon_count == 0
    pbMessage(_INTL("你的手上没有可以战斗的精灵！") + "\1")
...
def pbItemBall(item, quantity = 1)
  ...
  if item.is_machine?   # TM or HM
    if quantity > 1
      pbMessage(_INTL("你捡到了{1}个\\c[1]{2} {3}\\c[0]！", quantity, itemname, GameData::Move.get(move).name))
    else
...
def pbReceiveItem(item, quantity = 1)
  ...
  if item == :DNASPLICERS
    ...
    pbMessage("\\me[#{meName}]" + _INTL("你获得了{1}个\\c[1]{2}\\c[0]！", quantity, itemname) + "\\wtnp[40]")
  elsif itemname.starts_with_vowel?
    pbMessage("\\me[#{meName}]" + _INTL("你获得了\\c[1]{1}\\c[0]！", itemname) + "\\wtnp[40]")
  else
    pbMessage("\\me[#{meName}]" + _INTL("你获得了\\c[1]{1}\\c[0]！", itemname) + "\\wtnp[40]")
