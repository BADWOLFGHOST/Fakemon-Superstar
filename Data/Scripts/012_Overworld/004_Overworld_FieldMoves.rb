...
def pbDive
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_DIVE, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("这里的水很深，精灵也许可以潜入水中。"))
    return false
  end
...
def pbSurfacing
  ...
  movefinder = $player.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_DIVE, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("光从上方洒下，精灵也许可以从这浮出水面。"))
    return false
  end
...
def pbHeadbutt(event = nil)
  ...
  movefinder = $player.get_pokemon_with_move(move)
  if !$DEBUG && !movefinder
    pbMessage(_INTL("也许有精灵在这棵树上，也许有精灵可以晃晃它。"))
    return false
  end
  if pbConfirmMessage(_INTL("也许有精灵在这棵树上。你想用头锤吗？"))
...
def pbRockSmash
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_ROCKSMASH, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("这是一块岩石，精灵也许能破坏它。"))
    return false
  end
...
def pbStrength
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_STRENGTH, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("这是一块巨石，但精灵也许能移动它。"))
    return false
  end
  pbMessage(_INTL("这是一块巨石，但精灵也许能移动它。") + "\1")
