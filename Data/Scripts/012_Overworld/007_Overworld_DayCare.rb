...
class DayCare
  ...
  def self.deposit(party_index)
    ...
    raise _INTL("队伍中索引{1}处没有精灵。", party_index) if pkmn.nil?
  ...
  def self.withdraw(index)
    ...
    slot = day_care[index]
    if !slot.filled?
      raise _INTL("位置{1}未找到精灵。", index)
    elsif $player.party_full?
      raise _INTL("队伍里没有能给精灵的位置了。")
    end
  ...
  def self.collect_egg
    ...
    raise _INTL("无法生成精灵蛋。") if egg.nil?
  ...
  def pokemon_pair
    ...
    raise _INTL("找不到2只寄存的精灵。") if pkmn2.nil?
    return pkmn1, pkmn2
  end
