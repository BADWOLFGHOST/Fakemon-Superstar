...
def pbLearnMove(pkmn, move, ignore_if_known = false, by_machine = false, &block)
  ...
  if pkmn.egg? && !$DEBUG
    ...
    pbMessage(_INTL("黑暗精灵不能学习招式。"), &block)
    return false
  end
  pkmn_name = pkmn.name
...
def pbUseItem(bag, item, bagscene = nil)
  ...
  if useType == 1   # Item is usable on a Pokémon
    ...
    pbFadeOutIn do
      ...
      screen.pbStartScene(_INTL("要对哪只精灵使用？"), false, annot)
      loop do
        scene.pbSetHelpText(_INTL("要对哪只精灵使用？"))
...
def pbUseItemOnPokemon(item, pkmn, scene)
  ...
  if itm.is_machine?
    ...
    if pkmn.shadowPokemon?
      pbMessage(_INTL("黑暗精灵不能学习招式。")) { scene.pbUpdate }
  ...
  if pkmn.hasItem?
    ...
    if scene.pbConfirm(_INTL("想要交换这两个道具吗？"))
      ...
      if !$bag.add(pkmn.item)
        ...
        scene.pbDisplay(_INTL("背包已经满了，无法拿回精灵身上的道具。"))
