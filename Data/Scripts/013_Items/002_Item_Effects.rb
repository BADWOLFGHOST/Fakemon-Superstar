...
ItemHandlers::UseFromBag.addIf(:move_machines,
  ...
  proc { |item|
    ...
    next 0 if !pbConfirmMessage(_INTL("要让教{1}给精灵吗？",
...
ItemHandlers::UseOnPokemon.add(:GRACIDEA, proc { |item, qty, pkmn, scene|
  if !pkmn.isSpecies?(:SHAYMIN) || pkmn.form != 0 ||
    ...
    scene.pbDisplay(_INTL("不能给倒下的精灵使用。"))
    next false
  end
  pkmn.setForm(1) do
    scene.pbRefresh
...
ItemHandlers::UseOnPokemon.add(:DNASPLICERS, proc { |item, qty, pkmn, scene|
  ...
  # Fusing
  chosen = scene.pbChoosePokemon(_INTL("和哪个精灵融合？"))
  ...
  if pkmn == other_pkmn
    ...
    scene.pbDisplay(_INTL("不能与倒下的精灵融合。"))
    next false
  elsif !other_pkmn.isSpecies?(:RESHIRAM) && !other_pkmn.isSpecies?(:ZEKROM)
    scene.pbDisplay(_INTL("不能与这个精灵融合。"))
    next false
  end
  newForm = 0
...
ItemHandlers::UseOnPokemon.add(:NLUNARIZER, proc { |item, qty, pkmn, scene|
  ...
  # Fusing
  chosen = scene.pbChoosePokemon(_INTL("与哪个精灵融合？"))
  next false if chosen < 0
  other_pkmn = $player.party[chosen]
  if pkmn == other_pkmn
    scene.pbDisplay(_INTL("不能与自己融合。"))
    ...
    scene.pbDisplay(_INTL("不能与濒死的精灵融合。"))
    next false
  elsif !other_pkmn.isSpecies?(:LUNALA)
    scene.pbDisplay(_INTL("不能与精灵融合。"))
    next false
  end
  pkmn.setForm(2) do
    pkmn.fused = other_pkmn
