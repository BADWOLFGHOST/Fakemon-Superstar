...
class PokemonPokedexInfo_Scene
  ...
  def pbGetAvailableForms
    ...
    ret.each do |entry|
      if entry[0]   # Alternate forms, and form 0 if no gender differences
        ...
        case entry[1]
        when 0 then entry[0] = _INTL("日耀")
        when 1 then entry[0] = _INTL("月辉")
        else
          entry[0] = (multiple_forms) ? _INTL("形态") : _INTL("元素不明")
        end
      end
  ...
  def drawPageInfo
    ...
    if @show_battled_count
      textpos.push([_INTL("战斗次数"), 314, 164, :left, base, shadow])
      ...
      textpos.push([_INTL("重量"), 314, 196, :left, base, shadow])
    end
    if $player.owned?(@species)
      # Write the category
      textpos.push([_INTL("{1} 精灵", species_data.category), 246, 80, :left, base, shadow])
      ...
      if !@show_battled_count
        ...
        if System.user_language[3..4] == "US"   # If the user is in the United States
          ...
          textpos.push([_ISPRINTF("{1:.1f} 米", height / 10.0), 470, 164, :right, base, shadow])
          textpos.push([_ISPRINTF("{1:.1f} 千克", weight / 10.0), 482, 196, :right, base, shadow])
        end
      end
      ...
      # Write the category
      textpos.push([_INTL("??? 精灵"), 246, 80, :left, base, shadow])
      # Write the height and weight
      if !@show_battled_count
        if System.user_language[3..4] == "US"   # If the user is in the United States
          ...
          textpos.push([_INTL("??? 米"), 470, 164, :right, base, shadow])
          textpos.push([_INTL("??? 千克"), 482, 196, :right, base, shadow])
        end
      end
    end
    # Draw all text
  ...
  def drawPageArea
    ...
    if points.length == 0
      ...
      textpos.push([_INTL("栖息地未知"), Graphics.width / 2, (Graphics.height / 2) + 6, :center, base, shadow])
    end
