...
class PokemonPartyPanel < Sprite
  ...
  def draw_gender
    ...
    gender_text  = (@pokemon.male?) ? _INTL("耀") : _INTL("辉")
    base_color   = (@pokemon.male?) ? Color.new(255, 213, 8) : Color.new(189, 222, 230)
    shadow_color = (@pokemon.male?) ? Color.new(213, 131, 8) : Color.new(106, 148, 148)
    pbDrawTextPositions(@overlaysprite.bitmap,
...
class PokemonParty_Scene
  def pbStartScene(party, starthelptext, annotations = nil, multiselect = false, can_access_storage = false)
    ...
    @sprites["storagetext"] = Window_UnformattedTextPokemon.new(
      @can_access_storage ? _INTL("（按下Shift）开启精灵存储") : ""
    )
  ...
  def pbChoosePokemon(switching = false, initialsel = -1, canswitch = 0)
    ...
    loop do
      ...
      if Input.trigger?(Input::SHIFT) && @can_access_storage && canswitch != 2
        pbPlayDecisionSE
        pbFadeOutIn do
...
class PokemonPartyScreen
  ...
  def pbPokemonGiveScreen(item)
    @scene.pbStartScene(@party, _INTL("给哪个精灵？"))
  ...
  def pbPokemonGiveMailScreen(mailIndex)
    ...
    if pkmnid >= 0
      ...
      if pkmn.hasItem? || pkmn.mail
        pbDisplay(_INTL("精灵已经携带道具了，不能携带邮件。"))
      elsif pkmn.egg?
  ...
  def pbPokemonMultipleEntryScreenEx(ruleset)
    ...
    @scene.pbStartScene(@party, _INTL("选择精灵并确认。"), annot, true)
    loop do
      realorder = []
      @party.length.times do |i|
      ...
      @scene.pbSetHelpText(_INTL("选择精灵并确认。"))
      pkmnid = @scene.pbChoosePokemon
      addedEntry = false
      ...
      if cmdEntry >= 0 && command == cmdEntry
        if realorder.length >= ruleset.number && ruleset.number > 0
          pbDisplay(_INTL("最多只能让{1}只精灵参加。", ruleset.number))
        else
        ...
        @scene.pbSummary(pkmnid) do
          @scene.pbSetHelpText((@party.length > 1) ? _INTL("选择一只精灵。") : _INTL("选择精灵或取消。"))
        end
      end
    end
    @scene.pbEndScene
  ...
  def pbChooseAblePokemon(ableProc, allowIneligible = false)
    ...
    @scene.pbStartScene(
      @party,
      (@party.length > 1) ? _INTL("选择一只精灵。") : _INTL("选择精灵或取消。"),
      annot
    )
    loop do
      @scene.pbSetHelpText(
    ...
    loop do
      ...
      when Integer   # Hidden move's index
        ...
        if [:MILKDRINK, :SOFTBOILED].include?(move.id)
          ...
          @scene.pbSetHelpText(_INTL("对哪个精灵使用？"))
          ...
          loop do
            ...
            if party_idx == old_party_idx
              ...
              pbDisplay(_INTL("{1}不能对精灵蛋使用！", movename))
            elsif newpkmn.fainted? || newpkmn.hp == newpkmn.totalhp
              pbDisplay(_INTL("{1}不能对这只精灵使用！", movename))
            else
              pkmn.hp -= amt
          ...
          if pbConfirmUseHiddenMove(pkmn, move.id)
            ...
            if move.id == :FLY
              ...
              @scene.pbStartScene(
                @party, (@party.length > 1) ? _INTL("选择一只精灵。") : _INTL("选择精灵或取消。")
              )
              next
            end
            return [pkmn, move.id]
...
MenuHandlers.add(:party_menu, :summary, {
  ...
  "effect"    => proc { |screen, party, party_idx|
    screen.scene.pbSummary(party_idx) do
      screen.scene.pbSetHelpText((party.length > 1) ? _INTL("选择一只精灵。") : _INTL("选择精灵或取消。"))
    end
  }
})
...
MenuHandlers.add(:party_menu, :change_ball, {
  ...
  "effect"    => proc { |screen, party, party_idx|
    ...
    selected_cmd = screen.pbShowCommands(_INTL("选择一种精灵球来替换。\n现在的是：{1}", current_ball_name), commands, current_cmd)
    next if selected_cmd < 0
    new_ball = balls[selected_cmd]
    if new_ball.id == current_ball_id
      pbMessage(_INTL("不能选择一样的精灵球来替换。"))
      next
    end
    $bag.remove(new_ball.id)
    pkmn.poke_ball = new_ball.id
    pbMessage(_INTL("精灵已被重新捕捉进{1}里了。", new_ball.name))
    screen.pbRefreshSingle(party_idx)
  }
})
...
def pbChoosePokemon(variableNumber, nameVarNumber, ableProc = nil, allowIneligible = false)
  ...
  pbFadeOutIn do
    ...
    if ableProc
      ...
      screen.pbStartScene(_INTL("选择一只精灵。"), false)
