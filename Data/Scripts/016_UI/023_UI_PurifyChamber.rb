...
#    end
  ...
  def pbPlace(pkmn, position)
    ...
    if position == 0
      if pkmn.shadowPokemon?
        ...
        @scene.pbDisplay(_INTL("只有黑暗精灵才能放在这里。"))
        return false
      end
    elsif position >= 1
      if pkmn.shadowPokemon?
        @scene.pbDisplay(_INTL("不能在这放置黑暗精灵。"))
        return false
      else
        oldpkmn = PurifyChamberHelper.pbGetPokemon(@chamber, position)
        if oldpkmn
  ...
  def pbOnPlace(pkmn)
    ...
    if @chamber.setCount(set) == 0 && @chamber.isPurifiableIgnoreRegular?(set)
      ...
      @scene.pbDisplay(
        _INTL("{1}准备好解开束缚了，但是仪式至少还要再有一只正常的精灵才能开始。",
              pkmn.name)
      )
    end
  end
  ...
  def pbOpenSetDetail
            ...
            if pos
              ...
              if newpkmn
                if (newpkmn.shadowPokemon?) == (curpkmn.shadowPokemon?)
                  ...
                  @scene.pbDisplay(_INTL("这个精灵不能放在那里。"))
                end
              end
            end
          end
    ...
    if pbCheckPurify
      @scene.pbDisplay(_INTL("有精灵准备好解开心灵的束缚！") + "\1")
  ...
  def pbDoPurify
    ...
    purifiables.length.times do |i|
      ...
      if (i + 1) != purifiables.length
        @scene.pbDisplay(_INTL("还有精灵准备好解开心灵的束缚！") + "\1")
