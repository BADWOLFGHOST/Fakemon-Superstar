...
def pbTrainerTypeEditorNew(default_name)
  ...
  base_money = pbMessageChooseNumber(_INTL("设置击败训练家赢得金钱的倍数。"), params)
  # Construct trainer type hash
  tr_type_hash = {
    :id         => id.to_sym,
    :name       => name,
...
module TrainerBattleProperty
  ...
  def self.set(settingname, oldsetting)
    ...
    properties = [
      ...
      [_INTL("版本"),      LimitProperty.new(9999), _INTL("用于区分具有相同名称和训练家类型的训练家的编号。")],
      [_INTL("战败文本"),    StringProperty,          _INTL("当训练家被击败时显示的信息。")]
    ]
    Settings::MAX_PARTY_SIZE.times do |i|
      properties.push([_INTL("精灵{1}", i + 1), TrainerPokemonProperty, _INTL("训练家拥有的神奇宝贝。")])
    end
...
module TrainerPokemonProperty
  def self.set(settingname, initsetting)
    ...
    pkmn_properties = [
      [_INTL("物种"),       SpeciesProperty,                         _INTL("精灵的物种。")],
      [_INTL("等级"),         NonzeroLimitProperty.new(max_level),     _INTL("精灵的等级（1-{1}）。", max_level)],
      [_INTL("昵称"),          StringProperty,                          _INTL("精灵的昵称。")],
      [_INTL("形态"),          LimitProperty2.new(999),                 _INTL("精灵的形态。")],
      [_INTL("性别"),        GenderProperty,                          _INTL("精灵的性别。")],
      [_INTL("闪光"),         BooleanProperty2,                        _INTL("如果设置为真，则其为闪光精灵。")],
      [_INTL("超闪光"),    BooleanProperty2,                        _INTL("精灵是否为超闪光。（闪光并带有特殊的闪光动画）")],
      [_INTL("黑暗"),        BooleanProperty2,                        _INTL("如果设置为真，则其为黑暗精灵。")]
    ]
    Pokemon::MAX_MOVES.times do |i|
      pkmn_properties.push([_INTL("招式{1}", i + 1),
                            MovePropertyForSpecies.new(oldsetting), _INTL("精灵知道的招式，野生精灵请将所有招式留空。（使用 Z 键删除）")])
    end
    pkmn_properties.concat(
      [[_INTL("特性"),       AbilityProperty,                         _INTL("精灵的特性，会覆盖特性索引。")],
       [_INTL("特性索引"), LimitProperty2.new(99),                  _INTL("特性的索引。 \n0=第一特性\n1=第二特性\n2+=隐藏特性")],
       [_INTL("携带道具"),     ItemProperty,                            _INTL("精灵携带的道具")],
       [_INTL("性格"),        GameDataProperty.new(:Nature),           _INTL("精灵的性格。")],
       [_INTL("个体值"),           IVsProperty.new(Pokemon::IV_STAT_LIMIT), _INTL("精灵各项能力的个体值。")],
       [_INTL("努力值"),           EVsProperty.new(Pokemon::EV_STAT_LIMIT), _INTL("精灵各项能力的努力值。")],
       [_INTL("亲密度"),     LimitProperty2.new(255),                 _INTL("精灵的初始亲密度（0-255）")],
       [_INTL("精灵球"),     BallProperty.new(oldsetting),            _INTL("收服此精灵的精灵球。")]]
    )
...
def pbPokemonEditor
  ...
  pbListScreenBlock(_INTL("精灵物种"), SpeciesLister.new(0, false)) do |button, species|
    if species
      case button
      when Input::ACTION
