...
def pbDebugDayCare
  ...
  loop do
    ...
    if cmd == commands.length - 2   # Egg
      compat = $PokemonGlobal.day_care.get_compatibility
      if compat == 0
        pbMessage(_INTL("精灵不能繁殖。"))
      else
        msg = _INTL("精灵可以繁殖。（和睦性 = {1}）", compat)
        # Show compatibility
        if day_care.egg_generated
          ...
          when 0   # Collect egg
            if $player.party_full?
              pbMessage(_INTL("队伍已满，无法收集精灵蛋。"))
            else
              DayCare.collect_egg
              pbMessage(_INTL("收集了{1}个精灵蛋。", $player.last_party.speciesName))
              need_refresh = true
            end
          when 1   # Clear egg
          ...
          case pbMessage("\\ts[]" + msg, [_INTL("制造精灵蛋"), _INTL("取消")], 2)
...
class SpriteWindow_DebugRoamers < Window_DrawableCommand
  ...
  def drawItem(index, _count, rect)
    ...
    if index == self.itemCount - 2
      # Advance roaming
      self.shadowtext(_INTL("[所有精灵游走到新位置]"), rect.x, rect.y, nameWidth, rect.height)
    elsif index == self.itemCount - 1
      # Advance roaming
      self.shadowtext(_INTL("[清除当前所有游走精灵位置]"), rect.x, rect.y, nameWidth, rect.height)
    else
