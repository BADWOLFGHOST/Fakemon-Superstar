...
class ItemCraft_Scene
  ...
  def pbCraftItem(stock)
    ...
    loop do
      ...
      if Input.trigger?(Input::USE)
        ...
        pocket = item.pocket
        if pbConfirmMessage(_INTL("合成{1}个{2}？",volume*recipe_data.yield,itemname)) { pbUpdate }
          if canCraft?(index,volume)
            ...
            if added == quantity
              ...
              pbMessage(_INTL("你将{1}个{2}放入<icon=bagPocket{3}>\\c[1]{4}\\c[0]口袋。",
              ...
              pbPlayBuzzerSE
              pbMessage(_INTL("背包已满......")) { pbUpdate }
            end
          else
            pbPlayBuzzerSE
            pbMessage(_INTL("缺少合成所需的道具。")) { pbUpdate }
          end
        end
      end
  ...
  def pbRedrawItem(index,volume)
    ...
    textpos = [
    ["[C]：确认",4,Graphics.height-26,0,BASELIGHT,SHADOWLIGHT],
    ["[方向键]批量/翻页",Graphics.width/2,Graphics.height-26,2,BASELIGHT,SHADOWLIGHT],
    ["[X]：退出",Graphics.width-4,Graphics.height-26,1,BASELIGHT,SHADOWLIGHT]
    ]
...
def pbItemCrafter(stock,speech1=nil,speech2=nil)
  stock.each do |recipe|
    raise _INTL("{1}不可用。", recipe) if !GameData::Recipe.exists?(recipe)
  end
  stock.uniq!
  if stock.empty?
    pbMessage(_INTL("还没有学会如何合成道具。"))
    return
  end
  if pbConfirmMessage(_INTL("要开始合成吗？"))
    pbMessage(speech1 ? speech1 : _INTL("合成台开启！"))
    pbFadeOutIn {
  ...
  pbMessage(speech2 ? speech2 : _INTL("下次再合成吧。"))
end
