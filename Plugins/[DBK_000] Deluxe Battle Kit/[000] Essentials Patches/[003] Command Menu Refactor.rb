  ...
  def refreshButtonNames
    ...
    @buttons.each_with_index do |button, i|
      ...
      if GET_MOVE_TEXT_COLOR_FROM_MOVE_BUTTON && moves[i].display_type(@battler)
        color_x = 8 + (button.src_rect.x / 2).floor
        moveNameBase = button.bitmap.get_pixel(color_x, button.src_rect.y + 23)
      end
      base   = @customUI ? @base_color   : moveNameBase
      shadow = @customUI ? @shadow_color : TEXT_SHADOW_COLOR
      textPos.push([moves[i].name, x, y, :center, moveNameBase, Color.new(0,0,0,0)])
    end
...
class Battle::Scene
  ...
  def pbCommandMenu(idxBattler, firstAction)
    ...
    runCommand = (shadowTrainer) ? _INTL("呼唤") : (firstAction) ? _INTL("逃跑") : _INTL("取消")
    hasCheer = defined?(@battle.cheerMode) && @battle.cheerMode
    if hasCheer
      runCommand = _INTL("应援")
      mode = 5
    elsif @battle.launcherBattle?
      bagCommand = _INTL("发射器")
    ...
    if battler.effects[PBEffects::Encore] > 0
      if !move.powerMove? && move.id != battler.effects[PBEffects::EncoreMove]
        if showMessages
          ...
          pbDisplayPaused(_INTL("{1}因为对手的再来一次而只能使用{2}！", battler.name, encoreMove))
        end
        return false
      end
    end
