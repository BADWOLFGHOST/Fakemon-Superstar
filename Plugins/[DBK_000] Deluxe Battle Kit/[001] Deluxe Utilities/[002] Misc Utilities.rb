...
class Battle::Battler
  ...
  def pbCheckFormOnHeldItemChange
    ...
    if isSpecies?(:DIALGA)
      ...
      pbSimpleFormChange(newForm, _INTL("{1}的样子发生了变化！", pbThis))
    end
    #---------------------------------------------------------------------------
    ...
    if isSpecies?(:PALKIA)
      ...
      pbSimpleFormChange(newForm, _INTL("{1}样子发生了变化！", pbThis))
    end
    #---------------------------------------------------------------------------
    ...
    if isSpecies?(:ARCEUS) && self.ability == :MULTITYPE
      ...
      pbSimpleFormChange(newForm, _INTL("{1}转变为{2}属性！", pbThis, type.name))
    end
    #---------------------------------------------------------------------------
...
class PokemonEvolutionScene
  def pbBattleEvolution(battler)
    ...
    pbPlayDecisionSE
    @pokemon.play_cry
    @sprites["msgwindow"].text = _INTL("{1}的样子……！", battler.pbThis)
  ...
  def pbBattleEvolutionSuccess(battler)
    ...
    if battler.pbOwnedByPlayer?
      ...
      if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && !was_owned &&
        ...
        pbMessageDisplay(@sprites["msgwindow"],
                         _INTL("{1}的资料被添加到精灵图鉴里了！", newspeciesname)) { pbUpdate }
